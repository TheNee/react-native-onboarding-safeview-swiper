...
class Onboarding extends Component {
	...
	getItemLayout = (data, index) => (
		{ length: this.props.pages.length, offset: this.props.pages.length * index, index }
	)
	...
	render() {
		...
		this.props.onBoardingState(this.state)

		return (
			<SafeAreaView
		style={{ flex: 1, backgroundColor, justifyContent: 'center' }}>
	<StatusBar barStyle={this.state.gone ? 'default' : barStyle} backgroundColor={shadeColor(currentBackgroundColor, -0.20)} animated/>
		<FlatList
		ref={list => {
			this.flatList = list;
		}}
